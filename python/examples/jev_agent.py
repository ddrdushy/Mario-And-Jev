...
from nesenv.jev import DEFAULT_MODEL, HeuristicPolicy, JevPolicy, play_episode
...
def main() -> int:
    ...
    p.add_argument("--policy", choices=["jev", "heuristic"], default="jev")
    p.add_argument("--model", default=DEFAULT_MODEL)
    p.add_argument("--max-decisions", type=int, default=3000, help="hard cap on API calls")
    args = p.parse_args()

    rom = open(args.rom, "rb").read()
    policy = JevPolicy(model=args.model) if args.policy == "jev" else HeuristicPolicy()
